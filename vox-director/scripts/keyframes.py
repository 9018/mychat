...
beat has no "shots". Image generation on the gateway is synchronous — posters
are generated one at a time and downloaded to
<project>/keyframes/kf_<beat><shot>.jpg, url+path recorded onto each shot.
...
def run(project_dir):
    ...
    img_model = doc.get("image_model", IMAGE_MODEL)   # default agnes-image-2.1-flash
    ...
    for beat in doc["beats"]:
        for shot, key in shots_of(beat):
            if shot.get("keyframe_url"):        # already generated (e.g. reused wide) -> skip
                continue
            scene = shot["scene"]
            if shot.get("image_prompt"):          # user-authored prompt wins (direct passthrough)
                prompt = shot["image_prompt"]
            elif style == "collage":
                prompt = compose_collage_prompt(scene, beat.get("title_cn", ""), beat.get("title_en", ""),
                ...
                prompt = compose_keyframe_prompt(era, scene, beat.get("title_cn", ""),
