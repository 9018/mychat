...
EDIT_MODEL = "agnes-image-2.1-flash"   # gateway has no /edit endpoint — falls back to text-to-image
...
def run(project_dir):
    raise SystemExit(
        "C-roll is DISABLED on this gateway: it needs an image-EDIT model and "
        "background-removal to anchor a photo, and neither is provisioned. "
        "Use B-roll (topic → film) instead.")
    # (retained below for when the gateway provisions an edit model)
