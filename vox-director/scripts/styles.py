#!/usr/bin/env python3
"""
Per-era style bible for vox-director — the painterly styles remain from the original skill.
...
def image_params(model, aspect="16:9", resolution="1k"):
    """Per-model text-to-image params on the self-hosted gateway.
    agnes-image-* takes absolute `size` (like gpt-image); grok-imagine-*
    takes `aspect_ratio` (its endpoint rejects `size` outright)."""
...
# The agnes image-to-video model FOLLOWS the input image's aspect (no enum to
# pass). grok-imagine-video is not currently provisioned on the gateway; the
# enum below is the fallback if it ever comes back.
VIDEO_ASPECT_SUPPORT = {
    "agnes-video": None,
    "grok-imagine-video": ["16:9", "9:16", "1:1", "4:3", "3:4"],
}
