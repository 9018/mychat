...
def ff(args):
    base = ["ffmpeg", "-y", "-loglevel", "error", "-threads", os.environ.get("FFMPEG_THREADS", "8")]
...
def run(project_dir):
    ...
    # BGM is optional on this gateway (no music model): supply beats.json
    # `bgm_file` or run with narration only.
