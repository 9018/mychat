...
一个**通用 agent 技能**,后端全跑自托管 OpenAI 兼容网关 API、本地用 `ffmpeg` 合成,任何编码 agent(Claude Code、Codex 等)都能用。你给一句话选题,它给你一个 `mp4`。
...
| 用途 | 模型 |
|---|---|
| 关键帧 / 拼贴海报 | `agnes-image-2.1-flash` |
| 动效(图生视频) | `agnes-video-v2.0` |
| A-roll / 真人动效 | ❌ 网关未提供 |
| STT(A-roll) | ❌ 网关未提供 |
| 图片编辑(C-roll) | ❌ 网关未提供 |
| 旁白 | `mimo-v2.5-tts` |
...
然后设置网网关 key:
```bash
export OPENAI_API_KEY="sk-..."  # 自托管网关 key
export OPENAI_BASE_URL="http://10.0.1.108:18901/v1"
# 本网络必需(显式):10.0.1.108 只有经 SOCKS5 可达
export UPSTREAM_PROXY="socks5h://192.168.99.3:1080"
# grok-imagine-* 关键帧媒体取回(显式,无默认)
export GROK2API_BASE="http://10.0.1.108:8000"
export GROK2API_KEY="你的 Client Key"  # Grok2API 控制台签发
```
...
- **OpenAI 兼容网关** key(`OPENAI_API_KEY`)
...
全流程基于自托管 OpenAI 兼容网关构建——一个提示词,一条成片。
