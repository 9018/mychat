...
| **TTS 语音合成** | 预置音色 / 语音设计 / 语音克隆（支持麦克风录音） |
...
| * | `/v1/*` | AI API 反向代理 |
| * | `/agnesapi/*` | 视频状态查询代理 |
| GET/POST | `/api/mimo-key` | MiMo TTS API Key |
| * | `/mimo/*` | MiMo TTS API 反向代理 |
| GET | `/outputs/*` | 本地缓存媒体 |
...
|------|--------|------|
| `PORT` | `12301` | 监听端口 |
| `SSL_PORT` | `12300` | HTTPS 端口（用于麦克风录音） |
| `HOST` | `0.0.0.0` | 监听地址 |
...
TTS 模块通过 MiMo API 提供三种语音合成方式，支持中英文：

| 模式 | 说明 |
|------|------|
| **Quick TTS** | 选择 9 种预置音色（中英文），配合风格标签和自然语言描述生成语音 |
| **Voice Designer** | 用自然语言描述想要的音色风格，模型即时生成自定义人声 |
| **Voice Cloner** | 上传音频样本或直接**麦克风录音**，克隆音色后合成任意文本 |

### 麦克风录音

Voice Cloner 支持直接使用浏览器麦克风录音，无需提前准备音频文件。

- 点击 **🎤 开始录音** → 浏览器请求麦克风权限
- 录音中显示 🔴 计时器 + 红色停止按钮
- 停止后自动将录音转码为 WAV 格式（API 只支持 wav/mp3）
- 录音完成后图标变为 🎤，可移除后重新录制

### HTTPS 要求

浏览器安全策略要求 `getUserMedia`（麦克风访问）必须在**安全上下文**中运行：

- **localhost** / 127.0.0.1 → 豁免，HTTP 即可
- **局域网 IP**（如 192.168.x.x）→ 必须 HTTPS

服务器内置自签名 HTTPS 支持：

```
# HTTP 端口（默认）
http://192.168.99.4:12301/

# HTTPS 端口（录音可用）
https://192.168.99.4:12300/
```

首次访问 HTTPS 会提示证书风险，在浏览器中点 "Show Details" → "Visit This Website" 通过即可。
